    ...
    return (
      <div className="min-h-screen flex items-center justify-center text-white bg-[#070B14]">
        <div className="text-center">
          <p className="text-xl mb-3 text-red-400">⚠ Failed to load result</p>
            ...
            {data.strongAreas?.length ? (
              <div className="flex flex-wrap gap-2">
                {data.strongAreas.map((s: string, i: number) => (
                  <span key={i} className="px-3 py-1 text-sm rounded-full bg-green-500/20 text-green-300">
                    {s}
                  </span>
                ))}
              </div>
            ...
            {data.weakAreas?.length ? (
              <div className="flex flex-wrap gap-2">
                {data.weakAreas.map((w: string, i: number) => (
                  <span key={i} className="px-3 py-1 text-sm rounded-full bg-red-500/20 text-red-300">
                    {w}
                  </span>
                ))}
              </div>
          ...
          <div className="space-y-5">
            ...
            {data.answers.map((a: any, i: number) => (
              ...
              >
                ...
                {/* CODE BADGE */}
                {a.question?.toLowerCase().includes("function") && (
